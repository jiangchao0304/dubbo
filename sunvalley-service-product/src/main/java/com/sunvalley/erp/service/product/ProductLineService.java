...
import com.sunvalley.domain.erp.product.dto.ProductLineMappingDTO;
import com.sunvalley.erp.dao.product.*;
import com.sunvalley.erp.daoEX.product.ProductLineMappingExMapper;
...
public class ProductLineService {
    ...
    public void saveProductLine(int categoryid,int subCategoryId,int skuid,int sessionId)throws Exception{
        ...
        int opTyped=16; //更新
        productLineService.addLog(categoryid, opTyped,item.getSku(),"",1, "");


        //保存skuid 正式sku）与小类产品的关系
        ProductMapping productMappingx = new ProductMapping();
        ...
        productMappingx.setCreateUserId(SessionManager.getSysSession().getUserId());
        productMappingx.setType(1);
        productMappingMapper.insert(productMappingx);
        int opTypex=16; //更新
        productLineService.addLog(subCategoryId, opTypex,item.getSku(),"",1,"");
    }
